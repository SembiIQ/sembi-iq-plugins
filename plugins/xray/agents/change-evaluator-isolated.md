...
description: |
  Isolated, read-only variant of Xray change-evaluation. Runs the test-impact
  analysis in a separate context with a tools allowlist that cannot modify code
  or Xray data, and returns a single risk-sorted report. Prefer the
  ...
  user: "Do a read-only, sandboxed evaluation of my changes against the Xray Tests — run it separately and just report back."
  ...
  <example>
  Context: A large diff where keeping the Test-by-Test sweep out of the main conversation is desirable.
  user: "Evaluate this whole branch against Xray without cluttering our conversation — hand it off and summarize."
...
color: blue
tools: mcp__xray__describe_type, mcp__xray__get_test_count, mcp__xray__get_test, mcp__xray__get_tests, mcp__xray__get_expanded_test, mcp__xray__get_expanded_tests, mcp__xray__get_test_set, mcp__xray__get_test_sets, mcp__xray__get_test_plan, mcp__xray__get_test_plans, mcp__xray__get_test_execution, mcp__xray__get_test_executions, mcp__xray__get_test_run, mcp__xray__get_test_run_by_id, mcp__xray__get_test_runs, mcp__xray__get_test_runs_by_id, mcp__xray__get_precondition, mcp__xray__get_preconditions, mcp__xray__get_coverable_issue, mcp__xray__get_coverable_issues, mcp__xray__get_dataset, mcp__xray__get_datasets, mcp__xray__get_folder, mcp__xray__get_status, mcp__xray__get_statuses, mcp__xray__get_step_status, mcp__xray__get_step_statuses, mcp__xray__get_project_settings, mcp__xray__get_issue_link_types, Read, Glob, Grep, Bash
---
