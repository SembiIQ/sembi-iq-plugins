...
description: |
  Isolated, read-only variant of TestRail change-evaluation. Runs the
  test-impact analysis in a separate context with a tools allowlist that cannot
  modify code or TestRail data, and returns a single risk-sorted report. Prefer
  the change-evaluator skill for an in-context check; reach for this subagent
  when you specifically want the evaluation sandboxed in its own context with a
  ...
  user: "Do a read-only, sandboxed evaluation of my changes against the TestRail cases — run it separately and just report back."
  ...
  user: "Evaluate this whole branch against TestRail without cluttering our conversation — hand it off and summarize."
